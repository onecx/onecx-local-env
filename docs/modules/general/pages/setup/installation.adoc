...
This guide provides step-by-step instructions for setting up and installing `onecx-local-env`, the local development environment for OneCX. It covers prerequisites, installation steps, and initial configuration required to get `onecx-local-env` up and running on your local machine.
