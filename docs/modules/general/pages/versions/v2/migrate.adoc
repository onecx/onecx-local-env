...
This guide highlights the key differences between OneCX Local Environment xref:local-env:versions/v1/v1.adoc[v1] and xref:local-env:versions/v2/v2.adoc[v2], and explains how you can adapt your setup to ensure that it seamlessly works with xref:local-env:versions/v2/v2.adoc[v2].

== Network Rename

In xref:local-env:versions/v1/v1.adoc[v1], the network that all services connected to was named `example`.  
In xref:local-env:versions/v2/v2.adoc[v2], this network is now named `default`.
...
In xref:local-env:versions/v1/v1.adoc[v1]:
...
In xref:local-env:versions/v2/v2.adoc[v2]:
...
- For details on available profiles, see xref:local-env:versions/v2/v2.adoc[the v2 documentation].
