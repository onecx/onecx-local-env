...
-s::
Security authentication enabled, default not enabled (value is inherited from xref:./start.adoc[start-onecx.sh])
[CAUTION]
====
...
[TIP]
====
If you want only import tenant-specific data but do not working afterwards with security authentication then execute the xref:./start.adoc[start-onecx.sh] script without *-s* option again. This reset the security option on running services.
====
