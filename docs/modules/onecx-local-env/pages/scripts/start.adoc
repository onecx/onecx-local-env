...
There is no security context defined:
...
[TIP]
====
If data needs to be imported in a tenant-specific manner, then use the script xref:./import.adoc[import-onecx.sh] with the appropriate flags after starting the services.
====
