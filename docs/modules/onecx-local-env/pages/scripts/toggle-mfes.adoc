...
image::scripts/script_toggle-mfes.sh_list.png[width=550]

.Empty List - No Integrated Microfrontends
image::scripts/script_toggle-mfes.sh_list-empty.png[]
...
----
The *remoteEntry.js* should be loaded - the content is displayed. +
The *./toggle-mfes.sh* script does not check this!
====
...
In this example, the OneCX Workspace microfrontend is running both in Docker Compose (Service: *onecx-workspace-ui*) and locally (Service: *local_workspace-ui*). The local instance is used preferentially because it has a *higher* priority (*323*) than the Docker Compose instance (*52*).
