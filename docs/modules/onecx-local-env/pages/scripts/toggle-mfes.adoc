...
[CAUTION]
====
Microfrontends integrated using the methods described above take precedence over those running in Docker Compose. If the microfrontends started with Docker Compose are needed again, the locally integrated microfrontends must first be deintegrated using the `-d` option.
