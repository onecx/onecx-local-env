...
== Available Flags
The script accepts multiple flags to customize its behavior; one of each is required. *All options may only be used separately!*
...
[CAUTION]
====
Microfrontends integrated using the methods described above take precedence over those running in Docker Compose. If the microfrontends started with Docker Compose are needed again, the locally integrated microfrontends must first be deintegrated using the `-d` option. (A failover mechanism is not implemented due to different need of path stripping.)
====
