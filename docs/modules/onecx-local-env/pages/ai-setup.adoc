...
To access the Ollama AI service, you need a valid certificate. Follow the instructions in the documentation xref:./internal/custom-certificates.adoc[Custom Certificates].
...
. Prepare trust store for Ollama +
  Set up the trust store with the necessary certificates for secure communication with the Ollama AI service:
+
[source,bash]
----
./setup-truststore.sh
