...
Depends on your IT landscape, you need a valid certificate for the MCP Server. Follow the instructions in the documentation xref:./internal/custom-certificates.adoc[Custom Certificates].

MCP Server API Key (optional)::
This key is optional to access MCP Server functionalities within OneCX. Per default only the OneCX Documentation MCP Server is configured which is useable without an API Key: `onecx-docs-ai-dev.dev.one-cx.org/mcp`.
