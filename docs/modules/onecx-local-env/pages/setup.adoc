...
Once setup is done the environment can be started, see xref:./getting-started.adoc[Getting Started].
...
.Click to open for hosts entries
[%collapsible]
====
[source:text]
----
...
== Next Step
After completing the steps outlined in this guide, *onecx-local-env* is now set up and ready to run a local OneCX environment. Instructions on how to start, access, and stop the local environment can be found in the xref:./getting-started.adoc[Getting Started] guide.
