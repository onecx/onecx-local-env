...
.Solution icon:lightbulb[set=fas,role=yellow]
* Check if the hosts file is up-to-date. See xref:setup.adoc#hosts[Setup hosts] instructions. Add missing entries if necessary.
* Restart import with
+
[source,bash]
----
...
.Solution icon:lightbulb[role=yellow]
