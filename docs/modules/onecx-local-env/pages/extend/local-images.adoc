...
. Prepare SQL script with content
+
[source, sql]
----
...
. Execute the SQL script in Postgres container
+
[source, bash]
----
...
Before start the services, a word about the organization of the services:
...
These connections are defined by the "depends on" settings in the Docker Compose file. Therefore, it is sufficient to start only the UI service, which will automatically start the backend services as well.
