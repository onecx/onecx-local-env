...
If the `keycloak-app` service is reported as unhealthy, please try to re-run the docker compose command to start the service again. In some cases, Keycloak might take a bit longer to start up, and re-running the command can help resolve the issue. This issue is more common in xref:documentation:local-env:versions/v1/v1.adoc[OneCX Local Environment v1] due to differences in the performed health checks.
