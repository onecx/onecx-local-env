...
[quote]
____
This document only contains information specific to v2 of the OneCX Local Environment. Please make sure to also read the xref:../getting-started.adoc[Getting started guide] for general information and instructions that apply to all versions.
____


v2 is the current latest version of the local development environment for OneCX.

If you are currently using xref:../v1/v1.adoc[OneCX Local Environment v1] and want to migrate to v2, please refer to the xref:./migrate.adoc[Migrate to v2] guide for detailed migration instructions.
