...
#################################################################
# files 
files=$(ls ./microfrontends/*.json 2>/dev/null) || true
SKIP_MSG=""
if [[ -z "$files" ]]; then
  SKIP_MSG=" ==>${YELLOW} skipping${NC}: no files found"
fi

printf '%b\n' "$OLE_LINE_PREFIX${CYAN}Importing Microfrontends in Product Store via Operator${NC}\t$SKIP_MSG"
...
do
  ...
  mfe=$(printf '%s' "$filename" | cut -d'_' -f 3)
  ...
  if [[ "$status_code" =~ (200|201)$  ]]; then
    if [[ "${2:-}" == "true" ]]; then
      printf '    %b\n' "status: ${GREEN}$status_code${NC}, product: $product, app: $appid, mfe: $mfe"
    fi
  else
    printf '    %b\n' "${RED}status: $status_code, product: $product, app: $appid, mfe: $mfe ${NC}"
  fi
done
