...
#################################################################
# files 
files=$(ls ./slots/*.json 2>/dev/null) || true
if [[ -z "$files" ]]; then
  SKIP_MSG=" ==>${YELLOW} skipping${NC}: no files found"
fi

printf '%b\n' "$OLE_LINE_PREFIX${CYAN}Importing Slots in Product Store via Operator${NC}\t$SKIP_MSG"
  ...
  for ((i = 0 ; i < app_count ; i++ )); do 
    ...
    for ((j = 0 ; j < app_item_count ; j++ )); do 
      ...
      if [[ "$status_code" =~ (200|201)$  ]]; then
        if [[ "${2:-}" == "true" ]]; then
          printf '    %b\n' "status: ${GREEN}$status_code${NC}, product: $product, app: $app_id, slot: $app_item_name"
        fi
      else
        printf '    %b\n' "${RED}status: $status_code, product: $product, app: $app_id, slot: $app_item_name ${NC}"
      fi
    done
  done
done
