...
do
  ...
  if [[ "$status_code" =~ (200|201)$  ]]; then
    if [[ $2 == "true" ]]; then
      echo -e "  import: operator, status: ${GREEN}$status_code${NC}, product: $product, slot: $slot"
    fi
  else
    echo -e "${RED}  import: operator, status: $status_code, product: $product, slot: $slot ${NC}"
  fi
done
