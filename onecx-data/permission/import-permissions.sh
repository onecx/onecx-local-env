...
printf '%b\n' "$OLE_LINE_PREFIX${CYAN}Importing Permissions via Operator${NC}\t$SKIP_MSG"
...
do
  ...
  if [[ "$status_code" =~ (200|201)$  ]]; then
    if [[ "${2:-}" == "true" ]]; then
      printf '    %b\n' "status: ${GREEN}$status_code${NC}, product: $product, app: $appid"
    fi
  else
    printf '    %b\n' "${RED}status: $status_code, product: $product, app: $appid ${NC}"
  fi
done
