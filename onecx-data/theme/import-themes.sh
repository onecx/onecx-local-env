...
printf '%b\n' "$OLE_LINE_PREFIX${CYAN}Importing Themes via ExIm${NC}\t$SKIP_MSG"
...
do
  ...
  theme=$(printf '%s' "$filename" | cut -d '_' -f 2)
  ...
  if [[ "$status_code" =~ (200|201)$  ]]; then
    if [[ "${2:-}" == "true" ]]; then
      printf '    %b\n' "status: ${GREEN}$status_code${NC}, theme: $theme"
    fi
  else
    printf '    %b\n' "${RED}status: $status_code, theme: $theme ${NC}"
  fi
done
