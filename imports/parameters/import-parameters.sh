...
do
  ...
  status_code=`curl --write-out %{http_code} --silent --output /dev/null -X PUT -H 'Content-Type: application/json' "http://onecx-parameters-svc/operator/v1/parameters/$product/$appid" -d @$entry`
